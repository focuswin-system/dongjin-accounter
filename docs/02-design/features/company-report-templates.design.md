...
## 17. 회사도 자기 보고서를 관리한다 (2026-08-14)

### 축이 하나 더 생겼다 — 섞지 않는다
| 축 | 질문 | 정하는 사람 | 저장 위치 |
|---|---|---|---|
| **Entitlement** | 이 회사가 **쓸 수 있나** | 우리(공급자) | 플랫폼 DB `company_features` |
| **회사 설정** | 쓸 수 있는 것 중 **무엇을 쓸까** | 고객사 마스터 | **테넌트 DB** `report_prefs` |
| **Permission** | 이 사용자가 **볼 수 있나** | 고객사 마스터 | `role_perms` |

판정은 **AND** 다. `entitled AND enabled AND can(user)`.

**회사는 안 열린 양식을 켤 수 없다**(409). 열 수 있게 하면 고객이 유료 기능을 스스로 켜는 셈이라,
이 설계가 처음부터 피한 것이다. 반대로 **끄는 건 무엇이든 된다** — 기본 제공도, 계약이 끝난 것도.
안 쓰는 보고서를 자기 화면에서 치우는 건 그 회사가 정할 일이고, 목록이 짧아야 쓰는 사람이 찾는다.

### `report_prefs` 는 '끈 것만' 기록한다
행이 없으면 켜짐이다. 그래야 **우리가 새 양식을 열어줬을 때 회사가 따로 켜지 않아도 바로 보인다.**
켤 때는 행을 지운다 — '예전에 켠 기록'이 남아 판단을 흐리지 않게.

### 화면
- **고객사**: 환경설정 &gt; **보고서** (`settings_reports`, 마스터 전용).
  기본 제공/선택 제공 구분 · 상태(사용 중 / 꺼짐 / 미계약) · 끄기·켜기.
  미계약은 켜기 단추 대신 "문의 후 사용"이라고 적는다 — 누를 수 없는 단추를 보여주지 않는다.
- **운영 콘솔**: 구분별로 묶고(선택 제공을 맨 위 — 여기서 할 일이 있는 건 그것뿐),
  상태를 `사용 중 / 기간 지남 / **회사에서 끔** / 꺼짐` 으로 나눠 적는다.

### 콘솔이 회사 DB를 읽는 예외
콘솔은 원칙적으로 회사 DB를 보지 않는다(`routes/admin.js` 머리말). **여기만 예외다.**
없으면 콘솔은 '사용 중'이라 하는데 고객 화면엔 안 보이는 상태가 생기고, "왜 안 보여요"에 답할 수 없다.
읽는 값은 **화면 설정 한 컬럼**이고 회계 내용이 아니며, **읽기 전용**이다 —
회사가 끈 걸 우리가 대신 켜지 않는다. 그건 그 회사가 정할 일이다.

### 화면만 가리지 않는다
회사가 끄면 **데이터 API·엑셀도 403** 이다(`requireFeature`). 목록에서 빼는 것만으로는
URL 을 아는 사람이 그대로 부를 수 있다.

### 검증
17건 — 목록 구성 · 기본 제공 끄기 · 데이터/엑셀 차단 · **미계약 켜기 409** ·
미계약이어도 끄기는 됨 · **우리가 끄면 회사 설정과 무관하게 안 보임(계약이 위)** ·
없는 key/hidden 404. 콘솔 UI 11건, 단위 테스트 3건(축이 섞이지 않는지).

---

## 16. 자금관리표 — 대표 엑셀을 그대로 (2026-08-14)

받은 원본: `자금(현금)관리2026-샘플.xlsx` (월별 시트 12장).

### 원본의 구조
계좌별 `잔액 | 나갈 항목(이름(일자)) ×8 | 합계 | 차액` → 법인/개인/총합계 →
부채 현황 → 저축 현황 → **요약표**(보통계좌·저축·부채·지급예정·미지급 인건비·현금 과부족) →
퇴직자·현직원 미지급 → **입금 예정금액**.

### 더한 것: 들어온 돈
원본에는 <입금 예정금액>(들어올 돈)만 있고 **이미 들어온 돈이 없다.**
"이번 달에 얼마나 들어왔나"를 통장을 따로 열어 봐야 했다.
계좌 줄에 `들어온 돈 · 나간 돈` 두 열을 넣고, 요약과 KPI에도 실적을 함께 낸다.

### 숫자는 자금 현황과 같아야 한다 → lib 추출
`routes/fund-status.js` 안에 있던 집계 헬퍼(`openingBalances`·`actualsIn`·`debtStatus`·
`savingsStatus`·`laborStatus`·`closedState`)를 **`server/lib/fundStatus.js`** 로 옮기고
자금 현황과 자금관리표가 함께 쓴다. 두 벌로 두면 같은 회사 자금표가 화면마다 달라진다.
(`lib/cashReport.js` 를 자금일보·자금현황이 함께 쓰는 것과 같은 이유)
검증: 부채·저축·인건비·나갈 돈·들어온 돈·들어올 돈 **6개 합계가 두 화면에서 일치**.

### 엑셀은 원본과 배치가 다르다 — 이유
원본은 나갈 항목을 **가로 8칸**으로 펼쳤다. 사람이 손으로 채우던 표라 가능했다.
실데이터를 넣으니 계좌 하나에 항목이 **44개**까지 나온다(정기지출·상환 회차).
- 가로로 펼치면 44열 → 못 읽는다
- 8칸에서 자르면 **조용히 빠진 지출**이 생긴다 ← 자금표가 가장 하면 안 되는 일

그래서 **한 장(요약) + 명세 시트**로 나눴다. 아무것도 자르지 않는다.

| 시트 | 내용 |
|---|---|
| `2026년 8월` | 계좌별 요약(잔액·실적·나갈 건수·합계·차액) → 요약표 → 들어올 돈 → 들어온 돈 → 미지급 인건비 |
| `나갈 항목` | 전 항목 명세(계좌·일자·출처·내용·금액) + **계좌 미지정분 포함** |
| `부채 현황` / `저축·보증금 현황` | 기관·건명별 |

화면도 같은 이유로 나갈 항목을 **금액 큰 순 5개 + '외 N건'(눌러 펼침)** 으로 접는다.
숨긴 게 아니라 접은 것이고, 엑셀에는 전부 들어간다.

### 지어내지 않은 것
원본 요약표에는 **개인부채** 칸이 따로 있다(대표 개인 명의 차입).
지금 `loans` 에는 법인/개인 구분이 없다 — 계좌의 `owner` 같은 근거가 없다.
그래서 **부채·저축은 가르지 않고 합계로만** 내고, 화면·엑셀에 그 이유를 적었다.
구분이 데이터에 생기면 그때 가른다.

### 검증
자금 현황과 합계 6종 일치 · 계좌별+미지정 = 나갈 돈 합계 · 엑셀 명세 183행 = 실제 항목 수 ·
명세 합계 = 나갈 돈 · 안 켠 회사는 목록·데이터·엑셀 전부 차단.

---

## 15. P2 — 두 양식 실구현 (2026-08-14)

§14에서 "팔 수 없다"고 판정한 셋 중 **둘을 실데이터에 붙였다.** 방산 원가 보고서는 그대로 `hidden`.

### 계약별 수익 현황 — 서버 코드 0줄
`GET /api/contracts` 가 **이미 필요한 값을 다 계산해 준다**(`in_supply`·`cost_supply`·`profit`·`billed`·`ar_remain`).
여기서 SQL을 새로 짜면 계약 화면과 숫자가 갈린다 — 같은 계약의 손익이 화면마다 다르면 둘 다 못 믿게 된다.
그래서 **집계는 routes/contracts.js 한 곳에 두고 화면만 그린다.**

서버가 지켜 주는 규칙(다시 계산하면 안 되는 이유):
- 손익은 **공급가액 기준**. VAT 포함으로 계산하면 정확히 10% 부풀려진다(1.1S − 1.1C).
- 원가는 `cost_contract_id`(원가 귀속)로 붙은 지출이지 `contract_id`(근거 계약)가 아니다.
  외주비 한 건은 외주 매입계약의 '지급'이면서 동시에 프로젝트 매출계약의 '원가'다.
- 매입 계약은 `profit`·`cost` 가 `null` → **표에서 뺀다.** 섞으면 손익 열이 빈 줄로 남아 합계의 뜻이 사라진다.

화면에서 지킨 것:
- 받은 매출이 0인 계약은 이익률을 **계산하지 않는다**(`—`). 0으로 나눠 `NaN%` 를 찍던 게 §14의 결함이었다.
- 손실이면 막대를 안 그린다. 길이 0인 막대는 '적자'가 아니라 '데이터 없음'으로 읽힌다.
- 합계 이익률은 **합계 기준**이다(계약별 이익률의 산술평균이 아니다 — 작은 계약이 과대 반영된다).

### 세무사 전달용 자료 — 새 집계 + 엑셀
`server/lib/taxofficePack.js` 신설. 7종을 **실제로 센다**(예전엔 16·7·5·8·7·1·3 이 코드에 박혀 있었다).

| 항목 | 기준 |
|---|---|
| 월별 입출금 내역 | 완료된 거래만(입금완료/지급완료). 계좌·적요까지 — 세무사가 통장과 대조한다 |
| 지출결의서 | **승인 완료만.** 작성 중인 결의서를 넘기면 미확정 지출이 장부에 오른다. 날짜는 지급일 |
| 세금계산서 매출·매입 | 발행일 기준. 공급가·세액을 나눠 낸다 |
| 급여대장 | **달력월(payroll.month) 기준.** 급여는 '7월분'으로 신고하지 '6/26~7/25분'으로 신고하지 않는다 |
| 원천징수 대상 급여 | 공제가 있는 급여 명단. **신고서 서식은 만들지 않는다** — 서식·세율은 해마다 바뀌어 우리가 책임질 자리가 아니다 |
| 증빙 누락 지출 | 완료 지출 중 증빙 없는 건. 급여·차입·적금은 뺀다(원래 증빙이 없어, 안 빼면 진짜 누락이 묻힌다) |

- 달 구간은 **회계 마감일**을 따른다. 그래서 화면·엑셀 목차에 구간을 적는다 — 받는 쪽이 달력월로 오해하면 대조가 안 맞는다.
- **ZIP 대신 엑셀 한 권**(목차 + 7시트). 받는 쪽은 결국 풀어서 하나씩 열게 되고, 우리는 압축 라이브러리를 더 들여야 한다. `xlsx` 는 이미 쓰고 있다.
- 시트의 열은 화면 표와 같다(`SHEETS`) — 보이는 것과 받는 것이 다르면 대조가 안 된다.

### 구현 중 메운 구멍 — 데이터 API가 열려 있었다
목록에서 빼는 것만으로는 부족했다. **기능을 꺼도 `/api/reports/taxoffice` 는 그대로 200 이었다.**
URL 을 아는 사람은 부를 수 있고, 한 번 켰다 끈 회사는 브라우저 기록에 주소가 남는다.
권한 게이트(`perm.js`)는 `report` 자원 하나로만 판정하므로 여기까지 못 온다 — **양식 단위 판정은 라우트가 유일한 자리**다.
`requireFeature()` 로 데이터·엑셀 양쪽을 막았다(안 켜짐 403 / hidden 404).
설계서 §5.3 이 이미 "프런트를 믿지 않는다"고 적어 둔 것을, 실구현에서야 지켰다.

### 지금 열 수 있는 양식
| 양식 | scope | 상태 |
|---|---|---|
| 계약별 수익 현황 | `entitled` | **콘솔에서 회사별로 열 수 있다** |
| 세무사 전달용 자료 | `entitled` | **콘솔에서 회사별로 열 수 있다** |
| 방산 원가 보고서 | `hidden` | 여전히 빈 표 + `NaN%`. 실구현 전까지 못 켠다(회귀 테스트로 고정) |
