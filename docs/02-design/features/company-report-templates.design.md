...
### 지금 상태 — **화면 변화 0**
- 기존 7개가 **사원에게도 마스터에게도** 그대로. 잠금 카드는 하나도 없다.
- 미노출이던 3개는 `scope:'hidden'` — 카탈로그에 **등록만** 하고 아무에게도 안 내보낸다.
  화면이 검증되지 않았고 팔 준비도 안 됐기 때문이다.
- 판매 경로는 아직 없다. `company_features` 에 행을 넣으면 잠금이 풀린다(실동작 확인).

### scope 세 값
| scope | 사원 | 마스터 | 쓰는 때 |
|---|---|---|---|
| `all` | 보임 | 보임 | 기본 제공 |
| `entitled` | 사야 보임 | 잠금 카드로 보임 | **팔 때** |
| `hidden` | 안 보임 | 안 보임 | **아직 안 팔 때** ← 지금 그 3개 |

**`hidden` 을 '카탈로그에서 빼기' 대신 둔 이유**: 빼면 `REPORT_VIEWS` 에만 화면이 남아
**또 미아가 된다**(이 셋이 정확히 그 상태였다). 등록해 두고 안 내보내면 검사 [15]가 계속 지켜준다.
팔기로 하면 `'entitled'` 로 **한 단어만** 바꾸면 된다.
`hidden` 은 기능 키를 켜 줘도 안 나간다 — 파는 행위는 반드시 `entitled` 로 바꾸는 명시적 결정을 거친다.
