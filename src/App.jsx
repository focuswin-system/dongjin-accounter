...
import { ContractListScreen, ContractScreen } from './screens/Contract'
            ...
            return (
              <div key={node.id} style={{ marginTop: 4 }}>
                ...
                {open && node.sections.map(section => (
                  /* 라벨이 빈 섹션은 머리글을 안 그린다 — '거래내역 › 거래내역'처럼
                     한 겹이 헛도는 자리를 위해 nav.js 가 라벨을 비워 보낸다.
                     (브레드크럼 쪽은 NAV_PATH_OF 가 이미 단일 항목 섹션을 접는다.) */
                  <div key={section.label || `_${section.items[0]?.id}`}
                    className={`nav-section${section.label ? '' : ' nav-section-plain'}`}>
                    {section.label && <div className="nav-group-label nav-sub-label">{section.label}</div>}
