 ...
 *
 * 작은 화면(노트북)에서 열 하나가 두 줄로 접히는 일이 잦았다. 열 폭을 넓히려면
 * 다른 칸을 줄여야 하는데, 날짜는 **연도 두 자리를 버려도 뜻이 안 상한다** —
 * 회계 화면에서 보는 날짜는 거의 올해·작년이고, 정확한 값은 상세에서 보면 된다.
 * 두 글자를 줄이면 그만큼 이름 칸이 넓어져 접히는 줄이 사라진다.
 *
 * ⚠ 입력·저장에는 쓰지 않는다. **보여주기 전용**이다 —
...
export const Icon = {
  ...
  /* 즐겨찾기 별 — `filled` 로 담긴 상태를 나타낸다.
     담긴 것과 안 담긴 것을 **선 굵기나 색이 아니라 채움**으로 가른다.
     회색 빈 별과 진한 빈 별은 나란히 두면 구별이 안 된다(작은 아이콘이라 더 그렇다). */
  Star:    ({ filled, ...p }) => <I {...p} fill={filled ? "currentColor" : "none"}
             d={<path d="M12 3.6l2.6 5.3 5.8.85-4.2 4.1 1 5.75L12 16.9l-5.2 2.7 1-5.75-4.2-4.1 5.8-.85z"/>} />,
