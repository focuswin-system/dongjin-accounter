...
export const looksLikeBillingFile = (headers = []) =>
  /* ⚠ '수수료'는 넣지 않는다 — 해외 이용내역처럼 **정상 이용내역에도** 있는 머리글이라
       멀쩡한 파일에 경고가 뜬다. 청구내역에만 있는 말로만 판정한다. */
  headers.some(h => /청구\s*금액|이번\s*달\s*청구|당월\s*청구|결제\s*예정|할부\s*회차|잔여\s*(할부|회차|원금)|남은\s*(할부|회차)|^\s*원금\s*$/
    .test(String(h ?? '')))
