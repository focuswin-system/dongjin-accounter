...
/**
 * 이 엑셀이 **세금계산서 목록**인가 — 거래내역 업로드 화면에서 잘못 온 파일을 알아채는 데 쓴다.
 *
 * 계산서를 거래내역으로 올리면 오류 하나 없이 통과한다. 그리고 있지도 않은 입출금이
 * 수백 건 생기고(통장 잔액이 통째로 틀어진다), 정작 미수금·미지급금은 하나도 안 잡힌다.
 * 그래서 '틀렸다'고 막지 않고 **다른 자리가 있다**고 알린다.
 *
 * 판정은 계산서에만 있는 머리글로 한다 — 승인번호(24자리 발급번호)이거나,
 * 공급자와 공급받는자가 함께 있는 경우. 거래내역 양식에는 둘 다 없다.
 */
export const looksLikeTaxInvoice = (headers) => {
  const hs = (headers || []).map(h => String(h).replace(/\s/g, ''))
  if (hs.some(h => /승인번호/.test(h))) return true
  return hs.some(h => /^공급자/.test(h)) && hs.some(h => /공급받는/.test(h))
}
