...
function adaptTransaction(row) {
  return {
    ...
    /* 어음 만기 결제로 생긴 거래인가. 이 거래는 손익이 아니지만(비용은 어음을 끊을 때
       인식했다) **통장에서는 실제로 오갔다** — 입금내역·지급내역이 isPnl 로 거를 때
       이 표시가 없으면 통째로 빠진다. */
    noteId: row.note_id || '',
    noteNo: row.note_no || '',
...
export const api = {
  ...
  async getCommandIndex() {
    ...
    vendors.forEach(v => cmds.push({
      ...
      sub: [v.gubu === 'B' ? '매출' : v.gubu === 'C' ? '매입매출' : v.gubu === 'E' ? '기관' : '매입', v.type].filter(Boolean).join(' · '),
