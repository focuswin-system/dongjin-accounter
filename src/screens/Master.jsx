...
const CUSTOM_PANEL_TABS = new Set([
  "account", "card", "accountBalance", "payroll",
...
const MASTER_SECTIONS = {
  base: {
    ...
    groups: [
      ...
      // 반복거래는 기준정보가 아니라 돈 흐름이라 입출금 › 반복거래(screens/Repeat.jsx)에 있다.
    ],
  },
  settings: {
    title: "환경설정",
...
const VendorPanel = ({ embedded = false }) => {
  ...
  const toggleActive = async (v) => {
    ...
    /* 끈 뒤에도 **일이 남아 있으면 반드시 말한다.** 반복거래는 그대로 목록에 뜨는데
       거래처는 선택 목록에서 사라져, 들어온 돈을 붙일 곳이 없어진다(마산시니어클럽이 그랬다). */
    const p = res.pending
    const left = p && [
      p.contracts ? `진행중 주문 ${p.contracts}건` : null,
      p.recur_in ? `반복거래(입금) ${p.recur_in}건` : null,
      p.recur_out ? `반복거래(출금) ${p.recur_out}건` : null,
