...
const API_RESOURCES = {
  ...
  /* 어음은 재무관리의 '어음' 화면과 **수시 입금·수시 출금 안의 어음 탭**에서 부른다.
     ⚠ 청구서 쪽 자원을 빼면, 재무관리를 안 쓰는 회사에서 탭은 보이는데 403 이 된다. */
  '/api/notes':               ['finance_note', 'billing_issued', 'billing_received'],
  ...
  '/api/reports':             ['report'],
  /* 화면에 그려진 표를 서식 있는 엑셀로 바꿔 주는 통로(routes/export.js).
     DB 를 보지 않는다 — 받은 줄에 서식만 입혀 돌려준다. 그래서 자원은 **부르는 화면들**의
     것을 그대로 적는다(자원군 OR 판정). 새 화면이 쓰면 여기에 그 자원을 더한다. */
  '/api/export':              ['ledger', 'payment_run', 'purchase_status', 'report'],
 ...
 */
/* `/api/company`(조회 한 경로만) — 우리 상호·사업자번호·주소는 결의서·명세서·이체표 등 인쇄물
 * **열한 곳**의 머리글에 찍힌다. 환경설정 권한으로 막으면 그 권한이 없는 직원의 인쇄물에서
 * 공급자 칸이 조용히 비고, 첫 설정이 필요한지도 판정할 수 없다. 사업자등록증에 적힌 공개 정보다.
 * 수정·회기 미리보기·회계 처리 방식은 그대로 settings 권한이다. */
const LOOKUP_PATHS = ['/api/employees/options', '/api/company']
...
const ACTION_OVERRIDES = [
  ...
  /* 청구서 품목 다시 불러오기 — POST 지만 새 문서를 만들지 않는다. 있는 결의서의
     품목을 갈아끼우는 '수정'이다. 'create'로 두면 문서를 고칠 권한만 있는 사람이
     못 쓰고, 반대로 만들 권한만 있는 사람이 남의 결의서를 바꿀 수 있다. */
  { re: /\/reload-lines$/, action: 'edit' },
  /* 거래를 주문에 붙이거나 떼는 것 — POST 지만 새 거래를 만들지 않는다.
