...
router.post('/import/commit', async (req, res, next) => {
  ...
  try {
    ...
    for (const it of items) {
      ...
      /* 비목과 그에 딸린 계정과목.
         ⚠ **매입은 비목이 없으면 발행 전표의 차변 비용 줄이 통째로 빠진다**(lib/voucher.js) —
           차·대변이 안 맞는 전표가 임포트한 건수만큼 생긴다. 홈택스 엑셀엔 없는 칸이라
           여태 늘 비어 있었다. 이제 화면에서 한 번에 정하거나 엑셀에 열을 더해 보낼 수 있다.
         계정과목은 손으로 등록할 때와 **같은 함수**로 뽑는다(둘이 갈리면 같은 비목의
         분개가 등록 경로마다 달라진다). */
      ...
      if (it.action === 'update' && it.id) {
        ...
        if (Number(mcnt) > 0) {
          ...
          /* 비목·계정과목은 **비어 있을 때만** 채운다. 사람이 청구서에서 정해 둔 비목을
             엑셀의 기본값으로 덮으면, 공들여 분류한 것이 업로드 한 번에 지워진다. */
          await conn.execute(
...
router.get('/import/template', async (req, res, next) => {
  try {
    ...
    const guide = [
      ...
      ['• 상호 — 우리 반대편(매출이면 공급받는자, 매입이면 공급자)의 상호. 비어 있으면 그 행은 등록하지 않습니다.'],
      ['• 금액 — 합계금액이 0이거나 비어 있으면 그 행은 등록하지 않습니다.'],
      ['  (합계·공급가액·세액 중 둘만 있어도 나머지 하나는 계산해서 채웁니다)'],
      [''],
      ['[구분과 세액]'],
      ...
      ['  ※ 면세·영세로 판정되면 세액 칸에 값이 있어도 0으로 넣습니다(과세유형이 우선).'],
      [''],
      ['[승인번호 — 없어도 등록은 됩니다]'],
      ['• 중복 판정과 품목 묶기의 기준입니다. 있으면 같은 파일을 다시 올려도 두 번 등록되지 않아요.'],
      ...
      ['• 매출은 미수금(입금 예정), 매입은 미지급금(지급 대기)으로 잡히고 부가세 집계에 바로 반영됩니다.'],
      ['  전부 아직 안 받은/안 낸 상태로 들어갑니다 — 이 업로드는 돈을 움직이지 않습니다.'],
      ['  실제 입금·지급 처리는 청구서를 열어 정산(매칭)에서 하세요.'],
      ['• 마감한 달의 행은 등록하지 않고 건너뜁니다(이미 신고한 부가세 자료가 바뀌지 않게).'],
      ['  몇 건을 건너뛰었는지는 등록이 끝난 화면에 나옵니다.'],
      ['• 이미 입금·지급이 붙은 청구서를 덮어쓸 때는 금액·품목을 그대로 두고 승인번호만 채웁니다'],
      ['  (엑셀로 금액을 덮으면 정산 잔액이 어긋나기 때문입니다).'],
      [''],
      ['[비목 — 홈택스 엑셀에 없는 칸입니다]'],
      ['• 두 가지로 채울 수 있습니다: 엑셀에 "비목" 열을 만들어 적거나, 업로드 화면에서 한 번에 고르거나.'],
      ['  (둘 다 있으면 엑셀에 적힌 값이 이깁니다)'],
      ['• 비워 두면 매입은 전표의 비용 계정이 빈 채로 남아 차·대변이 안 맞습니다.'],
      ['  나중에 청구서를 열어 비목을 채우면 그때 맞춰집니다.'],
      ['• 이미 비목이 정해진 청구서를 덮어쓸 때는 그 비목을 그대로 둡니다(엑셀 기본값으로 덮지 않습니다).'],
      [''],
      ['[여기서 안 채워지는 것 — 나중에 손으로]'],
      ['• 주문(수주·발주) 연결과 입금·출금 예정 계좌는 비어 있습니다 — 필요하면 청구서에서 이어주세요.'],
    ]
    const wb = newBook()
