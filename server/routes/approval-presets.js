...
router.get('/', async (_, res, next) => {
  try {
    const [rows] = await pool.execute('SELECT * FROM approval_presets ORDER BY is_default DESC, sort_order, created_at')
...
router.post('/', async (req, res, next) => {
  try {
    ...
    const id = randomUUID()
    const [[{ maxOrder }]] = await pool.execute('SELECT COALESCE(MAX(sort_order),0) AS maxOrder FROM approval_presets')
    if (is_default) await pool.execute('UPDATE approval_presets SET is_default=0')
    await pool.execute(
...
router.patch('/:id/default', async (req, res, next) => {
  try {
    await pool.execute('UPDATE approval_presets SET is_default=0')
    const [r] = await pool.execute('UPDATE approval_presets SET is_default=1 WHERE id=?', [req.params.id])
    if (r.affectedRows === 0) return res.status(404).json({ error: 'Not found' })
    res.json({ ok: true })
  } catch (e) { next(e) }
})
...
router.delete('/:id', async (req, res, next) => {
  try {
    await pool.execute('DELETE FROM approval_presets WHERE id = ?', [req.params.id])
    res.json({ ok: true })
  } catch (e) { next(e) }
})
