...
router.post('/', async (req, res, next) => {
  ...
  try {
     ...
     *   '발행예정'에 하나 더** 뜬다 — 화면은 "아직 안 끊었다"고 말하는데 실제로는 끊었다.
     *   청구서 폼·거래 폼의 '이 이름으로 수주/발주 만들기'가 바로 이 경우고, 주문 금액을
     *   그 청구서 금액으로 잡기 때문에 **100% 겹친다**(실측 재현).
     *   기간·청구방식은 주문 화면에서 채우게 안내하고 있으니, 일정도 거기서 깔면 된다. */
    if (req.body.skip_schedule) {
      // 아무 일정도 깔지 않는다 — 근거가 되는 청구서·거래가 이미 있다.
    } else if (f.billing_mode === 'recurring') {
